...
# ---------------------------------------------------------------------------
# Default-profile orchestrator cross-board ACL (t_c452d9e7)
#
# The default profile is Hermes' cross-profile orchestrator and may
# address ANY board on this installation via the per-call ``board``
# argument without enumerating per-board ACL entries. Non-default
# profiles keep their existing profile-scoped board restrictions:
# they may only target the env-pinned board; any other slug is a
# misrouted call and is refused with a structured error.
#
# These tests pin both halves of the acceptance contract:
#   1. default profile: list/claim/write/read on multiple boards succeed
#   2. non-default profile: cross-board access is denied exactly as before
#
# The fixture below mirrors ``multi_board_env`` but exposes a way to
# pin the active profile by moving ``HERMES_HOME`` between the
# canonical root (``tmp/.hermes`` → active profile resolves to
# ``"default"`` via the equality check in ``get_active_profile_name``)
# and a profile subdir (``tmp/.hermes/profiles/<name>`` → resolves to
# ``"<name>"`` via the ``profiles/<name>`` relative path).
# ---------------------------------------------------------------------------


@pytest.fixture
def cross_board_profile_env(monkeypatch, tmp_path):
    """Isolated Hermes home with two boards seeded and helpers to flip
    the active profile between ``default`` and a named profile.

    Returns ``(info_dict, set_profile_fn)``. ``info_dict`` exposes the
    seeded task ids and DB paths; ``set_profile(name_or_None)`` moves
    ``HERMES_HOME`` to the matching path under ``profiles/<name>`` (a
    non-None name) or back to the canonical root (None). The DB seeds
    live in the canonical root so they're visible from both paths.
    """
    home = tmp_path / ".hermes"
    home.mkdir()
    profiles_dir = home / "profiles"
    profiles_dir.mkdir()
    # Seed under the canonical root BEFORE the test mutates HERMES_HOME.
    monkeypatch.setenv("HERMES_HOME", str(home))
    monkeypatch.delenv("HERMES_KANBAN_DB", raising=False)
    monkeypatch.delenv("HERMES_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    ...
    # Active profile is computed from the HERMES_HOME path — the
    # canonical root resolves to "default", a profile subdir resolves
    # to that profile's name. The kanban DBs live under the canonical
    # root and are reachable from BOTH paths (the kanban home() is
    # HERMES_HOME itself, not the profile dir), so we don't have to
    # re-seed for the non-default case.
    def set_profile(name: object) -> None:
        if name:
            profile_home = home / "profiles" / str(name)
            profile_home.mkdir(parents=True, exist_ok=True)
            monkeypatch.setenv("HERMES_HOME", str(profile_home))
        else:
            monkeypatch.setenv("HERMES_HOME", str(home))
        # Reset the kanban DB path cache so the next connect() resolves
        # against the freshly-set HERMES_HOME.
        kb._INITIALIZED_PATHS.clear()

    return info, set_profile


def test_default_profile_can_read_cross_board(cross_board_profile_env):
    """As the default profile, ``kanban_show`` succeeds on multiple
    boards via the per-call ``board`` override."""
    from tools import kanban_tools as kt

    info, set_profile = cross_board_profile_env
    set_profile(None)  # active profile == default

    # Cross-board show with explicit board= — the orchestrator bypass.
    out = kt._handle_show({"task_id": info["default_seed"], "board": "default"})
    d = json.loads(out)
    assert d["task"]["id"] == info["default_seed"]

    out = kt._handle_show({"task_id": info["alt_seed"], "board": "alt"})
    d = json.loads(out)
    assert d["task"]["id"] == info["alt_seed"]
    assert d["task"]["title"] == "seed-alt"


def test_default_profile_can_list_cross_board(cross_board_profile_env):
    """As the default profile, ``kanban_list`` returns each board's
    own tasks when given the matching ``board`` slug."""
    from tools import kanban_tools as kt

    info, set_profile = cross_board_profile_env
    set_profile(None)  # active profile == default

    # Default board lists the default seed but NOT the alt seed.
    out = kt._handle_list({"board": "default", "limit": 50})
    d = json.loads(out)
    ids = [t["id"] for t in d["tasks"]]
    assert info["default_seed"] in ids
    assert info["alt_seed"] not in ids

    # Alt board lists the alt seed but NOT the default seed.
    out = kt._handle_list({"board": "alt", "limit": 50})
    d = json.loads(out)
    ids = [t["id"] for t in d["tasks"]]
    assert info["alt_seed"] in ids
    assert info["default_seed"] not in ids


def test_default_profile_can_create_cross_board(cross_board_profile_env):
    """As the default profile, ``kanban_create`` writes to the board
    named in the ``board`` arg, not the env-pinned one."""
    from tools import kanban_tools as kt
    ...
    # The new task lives on the alt board; the default board can't see it.
    from hermes_cli import kanban_db as kb
    conn = kb.connect(board="alt")
    try:
        assert kb.get_task(conn, new_tid) is not None
    finally:
        conn.close()
...
def test_non_default_profile_cross_board_denied(cross_board_profile_env):
    """As a non-default profile, ``kanban_show`` against a foreign
    board is rejected with a structured error — the orchestrator
    bypass is explicit and does NOT loosen anything for them."""
    from tools import kanban_tools as kt

    info, set_profile = cross_board_profile_env
    set_profile("techlead")  # active profile == "techlead"

    # The alt board exists but the profile is pinned to the default
    # board via the env chain — kanban_show with ``board="alt"`` must
    # be refused.
    out = kt._handle_show({
        "task_id": info["alt_seed"],
        "board": "alt",
    })
    d = json.loads(out)
    assert "error" in d, out
    # The error message surfaces the orchestrator guidance so the
    # model can react instead of guessing.
    assert "default profile" in d["error"].lower()


def test_non_default_profile_pinned_board_allowed(cross_board_profile_env):
    """A non-default profile may still target its own env-pinned
    board (no regression). The ACL is a bypass for default, NOT a
    deletion of cross-board ACL for everyone."""
    from tools import kanban_tools as kt

    info, set_profile = cross_board_profile_env
    set_profile("techlead")  # active profile == "techlead"

    # The env chain resolves to "default"; the profile may read tasks
    # on its own pinned board (no explicit board=).
    out = kt._handle_show({"task_id": info["default_seed"]})
    d = json.loads(out)
    assert d.get("task", {}).get("id") == info["default_seed"]

    # And it may also explicitly name its own pinned board — the gate
    # only refuses FOREIGN boards.
    out = kt._handle_show({
        "task_id": info["default_seed"],
        "board": "default",
    })
    d = json.loads(out)
    assert d.get("task", {}).get("id") == info["default_seed"]
...
def test_worker_cross_board_denied_even_when_default_profile(
    cross_board_profile_env,
):
    """The orchestrator bypass applies to the default profile's
    ``orchestrator`` surface — a dispatcher-spawned worker is still
    pinned to one board by env, regardless of which profile the
    dispatcher's parent was running as."""
    from tools import kanban_tools as kt

    info, set_profile = cross_board_profile_env
    set_profile(None)  # active profile == default
    # Simulate a dispatcher-spawned worker on the default board.
    from hermes_cli import kanban_db as kb
    kb._INITIALIZED_PATHS.clear()
    conn = kb.connect()
    try:
        worker_task = kb.create_task(
            conn, title="worker-task", assignee="worker-d"
        )
        kb.claim_task(conn, worker_task)
    finally:
        conn.close()
    import os
    os.environ["HERMES_KANBAN_TASK"] = worker_task
    os.environ["HERMES_PROFILE"] = "worker-d"
    try:
        out = kt._handle_show({
            "task_id": worker_task,
            "board": "alt",
        ...
        d = json.loads(out)
        assert "error" in d, out
        assert "worker" in d["error"].lower()
    finally:
